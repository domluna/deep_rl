...
from deep_rl.trajectories import sample_traj, process_trajs
    ...
    with g.as_default(), tf.Session(config=config) as sess, tf.device('/cpu:0'):
        ...
        for i in range(FLAGS.iters):
            ...
            while timesteps_count < FLAGS.batch_size:
                ...
                trajs_count += 1
            process_trajs(trajs, vf_predict, FLAGS.gamma, FLAGS.gae_lambda)

            all_states = np.concatenate([t["obs"] for t in trajs])
            ...
            sess.run(pf_train_op, feed_dict={pf_input: all_states, advantages: all_advs, actions: all_acts})
